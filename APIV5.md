...
| categoryId | Long | ❌ | 分类ID |
| tagId | Long | ❌ | 标签ID |
| sortBy | String | ❌ | 排序方式：`newest`（默认，发布时间倒序）/ `hot`（综合热度：点赞×3+评论×2+浏览）/ `views`（浏览量倒序） |
...
```
GET /api/article/list?keyword=Spring&categoryId=1&sortBy=hot&pageNum=1&pageSize=10
```
...
| sortBy | String | ❌ | 排序方式：`newest`（默认）/ `hot`（综合热度）/ `views`（浏览量） |
| pageNum | Integer | ❌ | 当前页码，默认 1 |
| pageSize | Integer | ❌ | 每页条数，默认 10 |
...
```
GET /api/article/my?status=0&sortBy=newest&pageNum=1&pageSize=10
```
...
> 软删除，通知不会真正从数据库移除；只能删除自己的通知，删除他人通知无效
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| targetType | String | ✅ | 目标类型：`all`-全部用户，`user`-指定用户 |
| userId | Long | ❌ | 目标用户 ID，targetType=user 时必填 |
| title | String | ✅ | 通知标题 |
| content | String | ✅ | 通知正文 |
...
```json
{
  "targetType": "all",
  "title": "系统升级公告",
  "content": "系统将于今晚 22:00 进行维护升级，预计停服 30 分钟，请提前保存您的工作。"
}
```
...
```json
{
  ...
  "userId": 5,
  "title": "您的文章已通过审核",
  "content": "您的文章《Spring Boot 快速入门》已通过审核，现已对外展示。"
}
```
...
> 用于富文本编辑器插入图片。上传成功后返回可直接访问的图片 URL，将该 URL 插入富文本内容中即可。
>
> 图片存储在服务器本地 `uploads/yyyy/MM/` 目录下，可通过 `GET /uploads/yyyy/MM/filename.ext` 访问。
...
**限制：**
- 单文件最大 **10MB**
- 支持格式：`image/jpeg`、`image/png`、`image/gif`、`image/webp`、`image/svg+xml`

**请求示例（curl）：**

```bash
curl -X POST http://localhost:8080/admin/upload/image \
...
```json
{
  "code": 0,
  "message": "success",
  "data": "http://localhost:8080/uploads/2026/04/a3f2e1d0c4b5a678.png"
}
```

> `data` 为图片的完整访问 URL，可直接用于 `<img src>` 或富文本编辑器的图片插入
...
{
  "code": -1,
  "message": "不支持的图片格式，仅允许 JPG/PNG/GIF/WEBP/SVG",
  "data": null
}
```
...
const handler = function() {
  ...
  input.onchange = async () => {
    ...
    if (json.code === 0) {
      ...
      quill.insertEmbed(range.index, 'image', json.data);
    }
  };
  input.click();
};
